...
![Thread view](docs/thread-light.png)

A user turn, a collapsed reasoning panel, a tool call, markdown with inline
KaTeX, a table, a highlighted code block, and the message action row.
Captured from the production build in headless Chrome at 2×, at a viewport
tall enough to hold the whole conversation. Dark mode looks the part too, but
has no toggle — see [Known gaps](#known-gaps).
